...
Return as valid JSON in this exact format:
{
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a technology consultant. Generate comprehensive, accurate technology options for different project types. Always return valid JSON only, no additional text.'
            },
            {
              role: 'user',
              content: prompt
    ...
    const baseOptions = {
      'mobile-app': {
        frontend: {
          ...
          options: [
            { value: 'react-native', label: 'React Native', description: 'Cross-platform with JavaScript', pros: ['Single codebase', 'Native performance', 'Large community'], difficulty: 'Medium' },
            { value: 'flutter', label: 'Flutter', description: 'Google\'s cross-platform framework', pros: ['Fast development', 'Single codebase', 'Great performance'], difficulty: 'Medium' },
            { value: 'swift', label: 'Swift (iOS)', description: 'Native iOS development', pros: ['Best iOS performance', 'Apple ecosystem', 'Latest features'], difficulty: 'Hard' },
            { value: 'kotlin', label: 'Kotlin (Android)', description: 'Native Android development', pros: ['Best Android performance', 'Google preferred', 'Modern language'], difficulty: 'Hard' },
            { value: 'ionic', label: 'Ionic', description: 'Web technologies for mobile', pros: ['Web skills reuse', 'Rapid prototyping', 'Plugin ecosystem'], difficulty: 'Easy' }
          ]
        },
        backend: {
          ...
          options: [
            { value: 'firebase', label: 'Firebase', description: 'Google\'s mobile backend platform', pros: ['Real-time sync', 'Authentication', 'Push notifications'], difficulty: 'Easy' },
            { value: 'supabase', label: 'Supabase', description: 'Open source Firebase alternative', pros: ['PostgreSQL', 'Real-time', 'Self-hostable'], difficulty: 'Easy' },
            { value: 'node', label: 'Node.js', description: 'JavaScript backend', pros: ['Same language', 'Fast development', 'Large ecosystem'], difficulty: 'Medium' }
          ]
        },
        database: {
          title: 'Data Storage',
          ...
          options: [
            { value: 'firebase-firestore', label: 'Firestore', description: 'Google\'s NoSQL database', pros: ['Real-time sync', 'Offline support', 'Scalable'], difficulty: 'Easy' },
            { value: 'supabase', label: 'Supabase PostgreSQL', description: 'Managed PostgreSQL', pros: ['SQL database', 'Real-time', 'Row-level security'], difficulty: 'Easy' },
            { value: 'sqlite', label: 'SQLite', description: 'Local database', pros: ['Offline-first', 'No server needed', 'Fast queries'], difficulty: 'Easy' }
          ]
        },
        hosting: {
          ...
          options: [
            { value: 'app-stores', label: 'App Stores', description: 'iOS App Store and Google Play', pros: ['Maximum reach', 'Built-in payments', 'User trust'], difficulty: 'Medium' },
            { value: 'expo', label: 'Expo', description: 'React Native deployment platform', pros: ['Easy updates', 'No app store review', 'Quick testing'], difficulty: 'Easy' },
            { value: 'testflight', label: 'TestFlight + Play Console', description: 'Beta testing platforms', pros: ['User feedback', 'Gradual rollout', 'Testing tools'], difficulty: 'Medium' }
          ]
        }
      },
      'game': {
        frontend: {
          ...
          options: [
            { value: 'unity', label: 'Unity', description: 'Popular cross-platform game engine', pros: ['Visual editor', 'Cross-platform', 'Asset store'], difficulty: 'Medium' },
            { value: 'godot', label: 'Godot', description: 'Open source game engine', pros: ['Free and open', 'Lightweight', 'Easy scripting'], difficulty: 'Easy' },
            { value: 'phaser', label: 'Phaser', description: 'HTML5 game framework', pros: ['Web-based', 'JavaScript', 'Great for 2D'], difficulty: 'Easy' },
            { value: 'unreal', label: 'Unreal Engine', description: 'Professional game engine', pros: ['AAA quality', 'Visual scripting', 'Great graphics'], difficulty: 'Hard' }
          ]
        }
      }
    };
  ...
  const getWebAppOptions = () => ({
    frontend: {
      ...
      options: [
        { value: 'react', label: 'React', description: 'Popular and flexible', pros: ['Large community', 'Lots of resources', 'Very flexible'], difficulty: 'Medium' },
        { value: 'vue', label: 'Vue.js', description: 'Easy to learn', pros: ['Beginner friendly', 'Good documentation', 'Gentle learning curve'], difficulty: 'Easy' },
        { value: 'nextjs', label: 'Next.js', description: 'React with extra features', pros: ['SEO friendly', 'Fast performance', 'Built-in features'], difficulty: 'Medium' }
      ]
    },
    backend: {
      ...
      options: [
        { value: 'node', label: 'Node.js', description: 'JavaScript everywhere', pros: ['Same language', 'Large ecosystem', 'Good performance'], difficulty: 'Medium' },
        { value: 'none', label: 'No Backend', description: 'Static site only', pros: ['Fastest setup', 'Lowest cost', 'No complexity'], difficulty: 'Easy' }
      ]
    }
  });
...
Format your response as explanatory text, not a list.`;
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a tech consultant helping non-technical people choose the right technology stack. Explain choices in simple terms focusing on benefits and why they work well together.'
            },
            {
              role: 'user',
              content: prompt
      ...
      if (response.ok) {
        ...
        if (!techStack.frontend && !techStack.backend && !techStack.database && !techStack.hosting) {
  ...
  const generateRecommendedStack = (type: string, projectFeatures: string[]) => {
    const hasAuth = projectFeatures.some(f => f.toLowerCase().includes('auth') || f.toLowerCase().includes('login') || f.toLowerCase().includes('user'));
    const hasDatabase = projectFeatures.some(f => f.toLowerCase().includes('data') || f.toLowerCase().includes('store') || f.toLowerCase().includes('save'));
    ...
    if (type === 'mobile-app') {
      ...
      if (type === 'website' && !isComplex) {
        recommended.frontend = techOptions.frontend?.options?.find(opt => opt.value === 'vanilla')?.value || techOptions.frontend?.options?.[0]?.value;
      } else {
        recommended.frontend = techOptions.frontend?.options?.find(opt => opt.value === 'react')?.value || techOptions.frontend?.options?.[0]?.value;
      }

      if (hasAuth || hasDatabase || isComplex) {
        recommended.backend = techOptions.backend?.options?.find(opt => opt.value === 'node')?.value || techOptions.backend?.options?.[0]?.value;
      } else {
        recommended.backend = techOptions.backend?.options?.find(opt => opt.value === 'none')?.value || techOptions.backend?.options?.[0]?.value;
      }

      if (hasAuth || hasDatabase) {
        recommended.database = techOptions.database?.options?.find(opt => opt.value === 'supabase')?.value || techOptions.database?.options?.[0]?.value;
      } else {
        recommended.database = techOptions.database?.options?.find(opt => opt.value === 'none')?.value || techOptions.database?.options?.[0]?.value;
      }
    }
  ...
  const allSelected = Object.values(techStack).every(value => value);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="space-y-8">
        {Object.entries(techOptions).map(([category, categoryData]) => {
          const visibleOptions = showAllOptions[category] 
            ? categoryData.options 
            : categoryData.options.slice(0, 2);
          
          return (
            <div key={category} className="bg-gradient-to-br from-gray-800 to-purple-900/30 rounded-xl border border-purple-500/20 p-6 backdrop-blur-sm">
              <div className="mb-6">
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center mb-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${getDifficultyColor(option.difficulty)}`}>
                            {option.difficulty}
                          </span>
                        </div>
      ...
      <div className="mt-8 bg-purple-500/10 border border-purple-500/30 rounded-lg p-4">
        <div className="flex items-start">
          ...
          <div className="text-sm text-purple-200">
            ...
            <p>These technologies can often be changed later. Our recommendations are based on your project's specific needs, but you can always adjust as you learn more.</p>
          </div>
        </div>
      </div>
